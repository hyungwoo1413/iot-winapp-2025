...
### 윈앱 컨트롤3
- `NuemericUpDown` : 숫자 입력을 도와주는 컨트롤.
    - Nud~ 이름으로 시작
    - 나머지는 Button과 동일
...
- `RichTextBox` : 포맷팅이 가능한 텍스트박스
    ...
    - Rtb~ 로 시작
    - ScrollBars : 컨트롤 오른쪽과 하단에 스크롤바 생성
...
- `SaveFileDialog` : 저장위치에 파일명을 지정하는 다이얼로그 창 컨트롤
    - DlgOpen 이름으로 사용
    - 코딩으로 설정내용 작성
...
- 대부분 컨트롤 속성
    ...
    - Dock : 어디에 도킹시키는지 설정
        ...
        - 보통 컨테이너 컨트롤용으로 사용
    - Anchor와 Dock을 같이 사용해서 디자인
    - Font : 대표적인 폰트를 사용할 것
        - 폼에서 변경하면 하위 컨트롤은 같이 변경

### VS 개발팁
- 코드조각을 최대한 활용(코딩 양을 줄임, 오타 예방)
...
- `StatusStrip` : 상태표시줄 컨트롤. 서브 컨트롤 추가 가능
    ...
    - `DropDownButton` : 드롭다운버튼 추가
    - `SplitButton` : 버튼
    - SizeingGrip : 오른쪽 하단의 사이즈 조절 아이콘 표시

### 사용팁 

### C# 문법
- 기본 문법 - [소스](./day05/Day05Study/SyntaxWinApp01/FrmMain.cs)
    - 델리게이트(대리자), 이벤트
        - **대리자** : 메서드를 변수처럼 저장하고 호출할 수 있는 타입
            - C++ 함수포인터, 파이썬 콜백함수
            - 대리자는 실행할 메서드를 가지고 있고, 대리자를 호출하면 대리자가 메서드를 대신 호출해줌
            - 대리자는 호출할 메서드와 파라미터가 일치해야 함
            - 윈폼에서 버튼을 클릭하면, 직접 메서드를 호출하는게 아니고, C#이 사용자의 움직임을 체크하고 있다가 `클릭이벤트가 발생하면 그에 해당하는 메서드를 대신 실행(호출)`시켜줌
        ...
        - **이벤트** : 대리자의 특별한 형태. 어떤 일이 발생하면 연결된 메서드를 호출하는 것, PyQt의 `시그널`과 동일
            ...
            - event : delegate에게 '무슨 일 발생하면 실행해!'라고 해주는 역할
        ...
        - Form 오픈 이벤트 및 생성자
            ...
            - 2. `FrmMain_Load` 이벤트 : 폼이 로드(메모리 업)될때 처리할 내용
                - 생성자 이외에 초기화 할 것
                - 폼 오픈시 이벤트 중에서 가장 중요!
            - 3. FrmMain_Shown 이벤트 : 폼이 바탕화면에 그려질때 이벤트
                - 잘 안씀
        ...
        - 실제 실무에서는 FrmMain() 생성자, FrmMain_Load(), FrmMain_FormClosing() 으로 대부분 작업
...
- 고급 문법 - [소스](./day05/Day05Study/SyntaxWinApp02/FrmMain.cs)
    ...
    5. 제네릭 : Generic
        ...
        - Java, C# 등의 엄격한 데이터 타입 객체지향언어에 반드시 필요

        - 제네릭 메서드
        - 제네릭이 없으면 (실제, Console.cs의 WriteLine()이 증거)
        ```cs
        ...
        ```
        - 위 3개의 메서드를 하나의 메서드로 사용할 수 있음
        - 대문자 T는 아무거나 사용해도 무방. 대문자 한글자를 선호

        - 제네릭 클래스
        - where T : class -> 참조형(클래스)만 허용
        - where T : struct -> 값 형식(기본타입)만 허용
