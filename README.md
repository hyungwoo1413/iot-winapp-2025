...
- 순서
    1. 언어, 플랫폼, 프로젝트 선택
        ...
        - 플랫폼 : 모든 플랫폼
        - 프로젝트 : 데스크톱 선택
    2. Windows Forms 앱 선택(.NET Framework 안 적혀 있는 것)
    3. 프로젝트명 입력
    4. 추가 정보
        - 프레임워크 : .NET 8.0 선택
...
- debug/release 모드
    ...
    - release : 배포시 사용모드. 프로그램 최적화, 성능 향상
...
4. PyQt Designer와 거의 유사
    ...
    - StartPosition > CenterScreen 변경
    - MaximizeBox > False
    - FormBorderStyle > Sizalbe -> FixedSingle 변경
...
10. 새로 만든 컨트롤을 더블클릭
    - 이벤트 메서드가 자동 생성
    - 최초에 아무런 이벤트가 없을 때 더블클릭시 기본 이벤트 메서드 생성

11. 오류 발생
    - 폼 디자인의 컨트롤 이벤트와 로직 코드 상의 이벤트 처리 메서드 생성 상 불일치로 발생
    - 이벤트 이름은 생성되고, 이벤트 정의 메서드는 생성이 안됨
...
- 기본 문법 - C++, Java와 거의 동일. C++와의 차이점만 비교
    ...
    2. 자료형과 변수, Nullable
        ...
        - 닷넷타입은 여러언어에서 공통으로 사용하기 위해 추가된 기능
